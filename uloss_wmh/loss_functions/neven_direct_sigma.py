...
MODIFIES THE EXISTING NEVEN CODE TO PREDICT SIGMA DIRECTLY
"""
...
class CentreInstanceDistribution(Module):
    ...
    def forward(self, embedding_vector, sigma_mean, instance_mask):
        """
        ...
        phi_k = (
            -(embedding_vector - instance_centre.unsqueeze(-1).unsqueeze(-1)).pow(2) / (2*sigma_mean.unsqueeze(-1).unsqueeze(-1).pow(2))
        ).sum(dim=0).exp()
...
class NevenLoss(Module):
    ...
    def forward(self, offset_vectors, sigma_map, seed_map, label_instances):
        """
        ...
        assert sigma_map.shape[1] == 1 or sigma_map.shape[1] == offset_vectors.shape[1]
        ...
        # apply activation to the sigma loss
        torch.nn.functional.relu(sigma_map)
        ...
        for b in range(batch_size):
            ...
            for c in range(semantic_classes):
                ...
                for iid in torch.unique(labels_bc):
                    ...
                    instance_distribution = self.centre_instance_distribution(embeddings_b, sigma_mean, instance_mask)
